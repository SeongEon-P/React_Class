	...
	const handleRegister=(e) =>{
		...
		.catch((error)=>{
			...
			if(error?.response?.status ===409){
				setErrorMessage("username이 중복되었습니다.");
			}else{
    ...
    <div className='container mt-5'>
      <div className='card ms-auto me-auto p-3 shadow-lg custom-card'>
          ...
          <div className='form-group mb-2'>
            ...
            <input type='text' name='username' className='form-control' placeholder='username' value={user.username} onChange={handleChange} required />
            <div className='invalid-feedback'>유저네임을 입력해주세요</div>
          </div>
          <div className='form-group mb-2'>
            <label htmlFor='password'>패스워드</label>
            <input type='password' name='password' className='form-control' placeholder='password' value={user.password} onChange={handleChange} required />
